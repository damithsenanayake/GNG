    ...
    def fit_transform(self, X, colors):
        ...
        for i in range(maxiter):
            ...
            for x in X:
                ...
                if i >= 0.1 * maxiter:
                    del_edges = np.where((D - D.mean()) / D.std() > 1.)[0]  # 3.5 *(np.exp(-1.*(i)**2/(0.5*maxiter)*2)))[0]
        ...
        for _ in range(1000):
            for p in range(Y.shape[0]):
                ...
                pushdirs = np.array([self.t_dist(d_oths)]).T #* 25
                ...
                Y[p] += 0.02*((Y[y_neis]-Y[p])*pulldirs).sum(axis=0)
